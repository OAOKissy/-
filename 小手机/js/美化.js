...
function openChat(roleId){
  ...
  if (typeof currentRole.setupPatPatFeature === 'undefined') {
    ...
    // 将拍一拍的清理函数也添加到角色的清理函数列表中
    if (currentRole.setupPatPatFeature && typeof currentRole.setupPatPatFeature === 'function') {
      currentRole.cleanupFunctions.push(currentRole.setupPatPatFeature);
    }
  }
...
$("#send-btn").addEventListener("click", async ()=>{
  ...
  const now = Date.now();
  currentRole.messages.push({me:true, text: val, read: true, time: now});
  $("#chat-input").value = "";
  renderMessages();
  saveAll();
...
// open role editor from chat (edit current role)
$("#chat-api-btn").addEventListener("click", ()=>{
  if(!currentRole) return;
  editingRole = currentRole;
  $("#role-name").value = currentRole.name || "";
  $("#role-desc").value = currentRole.desc || "";
  $("#role-system").value = currentRole.system || "";
  $("#role-greeting").value = currentRole.greeting || "";
  $("#role-api-enabled").checked = !!currentRole.api?.enabled;
  $("#role-api-base").value = currentRole.api?.base || "";
  $("#role-api-key").value = currentRole.api?.key || "";
  $("#role-api-model").value = currentRole.api?.model || "";
  $("#role-api-model-custom").value = currentRole.api?.custom || "";
  $("#role-api-temp").value = currentRole.api?.temp ?? 0.8;
  $("#role-api-fields").classList.toggle("hidden", !currentRole.api?.enabled);
  $("#role-delete").classList.remove("hidden");
  // 显示角色头像或默认首字母
  if (currentRole.avatar) {
    $("#role-avatar-box").innerHTML = `<img src="${currentRole.avatar}" style="width:100%;height:100%;object-fit:cover;border-radius:8px;">`;
  } else {
    $("#role-avatar-box").innerText = (currentRole.name || "角色")[0];
  }
  $("#role-mask").style.display = "flex";
});
...
// 页面加载时创建图片查看器
window.addEventListener('load', createImageViewer);
...
function renderMessagesWithImageSupport() {
  ...
  currentRole.messages.forEach(m => {
    ...
    // 为拍一拍消息添加特殊样式
    if (m.isPat || m.isSystem) {
      msgContent.style.fontStyle = 'italic';
      msgContent.style.opacity = '0.7';
...
function initWallet() {
  ...
  function showWallet() {
    ...
    walletPage.style.zIndex = '60'; // 增加z-index以覆盖内容但不影响tabbar
    walletPage.style.boxSizing = 'border-box';
    walletPage.style.paddingTop = '38px'; // 为顶部状态栏留出空间
